...
# Reuse existing nodebuilder-couchdb container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-couchdb; then
	echo "Pulling NodeJS runtime..."
	buildah from --name nodebuilder-couchdb -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
	...
	--label="org.nethserver.images=docker.io/apache/couchdb:${couchdb_version}" \
	"${container}"
# Commit the image
